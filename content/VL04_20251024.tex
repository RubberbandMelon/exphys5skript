...
\subsection{Einführung: heteropolare und homöopolare Bindung}
    ...
    Spannend ist, wieso das Elektron vom Natrium zum Chlor übergeht: 
    es ist energetisch günstiger, die Chlor${}^-$-Schale aufzufüllen und Natrium${}^+$ zu bilden. 
    ...
    Die Ursache dafür ist quantenmechanischer Natur: 
    Die Wellenfunktionen der Elektronen interferieren konstruktiv zwischen den Protonen.
    Das führt zur einer Kompensation der Coulomb-Abstoßung und einer Energieabsenkung des gebundenen Zustandes.\\

    Interferieren die Wellenfunktionen der Elektronen jedoch destruktiv, dominiert die Coulomb'sche Abstoßung und es kommt zur Energieanhebung in einen anti-gebundenen Zustand.
    Typische Bindungsenergien sind $1$ - $\SI{5}{\electronvolt}$.
...
\subsection{Das Wasserstoff-Molekülion}
    ...
    \begin{minipage}{0.65\linewidth}
        ...
        \end{tikzpicture}
        \captionof{figure}{Schematische Darstellung des Wasserstoff-Molekülions \ce{H2+}. Eingezeichnet sind die Abstände $r_i$ zwischen den Protonen (+) und Elektronen (-).}
        \label{fig:1.3_H2+_ion}
    \end{minipage}\\

    Die Wellenfunktionen der Elektronen bei $a$, bzw. $b$ heißen $\varphi_\text a$ und $\varphi_\text b$, falls die Kerne weit voneinander entfernt sind und sich die Wellenfunktionen nicht gegenseitig beeinflussen.
    ...
    \begin{align}
        \nonumber
        &\int \varphi^2_\text{a/b} \mathrm{d}V = 1\\
        \nonumber
    ...
    \begin{itemize}
        \item Für $R_\mathrm{a,b}\to 0 $ geht $ \varphi_\mathrm{a}$ in $ \varphi_\mathrm{b}$ über, d.h. es gilt $ D= C$. Somit kann die Coulomb'sche Abstoßungsenergie der Kerne nicht kompensiert werden.
        \item Für wachsendes $R_\mathrm{a,b}$ nehmen sowohl
            $$
...
\paragraph{Vergleich Modell - Experiment}
    ...
    $$
    gewählt werden, mit dem Bohr'schen Radius $a_0$ und dem Variationsparameter $\alpha$, welcher für $\alpha=\num{1,24}$ die besten Vorhersagen trifft.
