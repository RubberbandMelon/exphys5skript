...
\subsection{Dielektrische Polarisierbarkeit}
    ...
    Üblicherweise ist die dielektrische Funktion $\tensor\epsilon$ ein 3x3-Tensor. In vielen Stoffen vereinfacht sich die dielektrische Funktion sogar zu einem Skalar $\epsilon$, welcher häufig \textbf{relative Dielektrizitätskonstante} genannt wird. 
...
\paragraph{Polarisierbarkeit $\bm\alpha$}
    ...
    In manchen Lehrbüchern definiert man
    $$
...
\paragraph{Clausius-Mossotti-Gleichung}
    ...
    \end{align}
    mit dem probenabhängigen \quickdef{Depolarisationsfaktor $\bm N$}.
...
\paragraph{Orientierungspolarisation}
    ...
    Bei hohen Temperaturen gilt $\mathrm{k}_{\mathrm{B}}T\gg \vec{p}_{\text{p}}\cdot\vec{E} = p_\text{p} E \cos\left( \theta \right) $ mit $p'_{\text{p}}= p_\mathrm{p} \overline{\cos\left( \theta \right) }$.
    \begin{verbal}
...
\paragraph{Bemerkungen}
    \begin{itemize}
    	...
    	\item Die genaue Rechnung betrachtet die mittlere Polarisation
            ...
            \begin{verbal_sidebyside}[7cm]
                ...
                Langevin-Funktion für $x\ll1$ eine gute Näherung.\\
                
                Für hohe Temperaturen $T$ wird das Argument klein und somit
                $$L\left(\frac{pE}{k_\text BT}\right) \approx \frac{pE}{3k_\text BT}.$$
                
                \tcblower
                \centering
                \includegraphics[width=\linewidth]{figures/vl2_langevin.jpg}
                \captionof{figure}{Langevin-Funktion $L(x)$ und gestrichelte lineare Approximation. Eigene Darstellung.}
    ...
    \begin{verbal_sidebyside}[0.5\linewidth]
        ...
        \captionof{figure}{Molpolarisierung $P_\text M$ gegen die inverse Temperatur $\frac 1T$ für verschiedene Stoffe \cite{Haken_Wolf_2006}.}
        \label{fig:vl2_molpolarisierung}
    \end{verbal_sidebyside}

\paragraph{Bemerkung 2}
    Die charakteristische Zeit für die Umorientierung eines Moleküls entspricht der Zeit zur Rotation eines Moleküls $\left( \sim \SI{e-12}{s} \right)$. Also können Moleküle mit Frequenzen $\ge \SI{100}{\giga \hertz}$, wie dem Wechselfeld von Strahlung, aufgrund ihrer Trägheit nicht mehr folgen.
