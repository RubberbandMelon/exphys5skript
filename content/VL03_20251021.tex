\subsection{Brechungsindex und Dispersion}
    ...
    $\mu$ und $\varepsilon$ sind frequenzabhängig, da die Verschiebungs- und Orientierungspolarisation ebenfalls von der Frequenz abhängen.
    ...
    \begin{minipage}{0.45\linewidth}
        \paragraph{Lorentz-Modell} Zur Beschreibung der Dispersion werden die Moleküle als gedämpfte harmonische Oszillatoren genähert. Dazu wird die Analogie zu einem Federpendel mit $\omega_0^2=\frac Dm$  gemäß \autoref{fig:I.2_federpendel} betrachtet.\\
	...
	Das E-Feld von Licht wird durch
	$$
	E = E_0 e^{i \omega t}.
	$$
	beschrieben. Die Differentialgleichung des getriebenen, gedämpften Oszillators ist
	\begin{equation}
		\label{2.19}
		m \ddot{x} + \gamma \dot{x} + m \omega_0^2x = \mathrm{e} E_0 e^{i \omega t}
	\end{equation}
	...
	\begin{align}
		...
		\label{eq:2.21}
		&= \left[ \frac{\mathrm{e} m\left( \omega_0^2 - \omega^2 \right) }{ m^2 \left( \omega_0^2 - \omega^2 \right)^2 + \left( \gamma \omega \right) ^2} -i \frac{\mathrm{e} \gamma\omega}{m^2\left( \omega_0^2 - \omega^2 \right)^2 + \left( \gamma \omega \right) ^2} \right]E_0.
	\end{align}
	Die Schwingung erzeugt ein Dipolmoment $p(t) = \mathrm{e}x(t)$. Die Dielektrizitätskonstante $ \varepsilon\left( \omega \right) $ lautet mit Gleichung \eqref{2.8} und \eqref{eq:2.11}
	\begin{equation}
		\label{eq:2.22}
	...
	\end{equation}
	wobei $n$ die Dispersion und $ \kappa$ die Absorption beschreiben. Die Frequenzabhängigkeit dieser beiden Komponenten $n$ und $\kappa$ ist in \autoref{fig:vl03_kramers_kronig} dargestellt.\\
	\begin{figure}[H]
	\centering
		\includegraphics[width=7cm]{figures/vl03/vl03_brechungsindex_kramers_kronig.png}
		\caption{Refraktion $n$ und Absorption $\alpha$, bzw. $\kappa$, sind abhängig von der Frequenz $\omega$ des induzierenden Lichts \cite{Haken_Wolf_2006}. Die Beziehung zwischen diesen beiden Größen wird durch die Kramers-Kronig-Beziehung beschrieben.}
	...
	\begin{figure}[H]
		...
		\caption{Dielektrizitätskonstante $\epsilon$ in Abhängigkeit der Frequenz $\omega$ der anregenden EM-Wellen \cite{Haken_Wolf_2006}. Die Polarisationseffekte $\chi$ treten nur bis zu bestimmten Frequenzen auf. Für sehr hohe Frequenzen gilt $\epsilon \to 1$, da die Polarisationseffekte verschwindent klein werden.}
...
\subsection{Moleküle in magnetischen Feldern}
	...
	\begin{equation}
		\chi_{m} = \mu-1
	\end{equation}
	berechnen lässt. Zusätzlich lässt sich die Feldstärke $\vec B$ durch die magnetische Erregung $\vec H$ mit
	\begin{equation}
		\label{eq:2.29}
		\vec{B} = \mu \mu_0 \vec{H}
	\end{equation} 
	...
	Insofern nur Diamagnetismus besteht, also $\vec{m}' = \vec m_{\text{ind}}$ gilt, können magnetische Momente immer noch durch ein externes magnetisches Feld induziert werden.

	\subsection{Diamagnetische Moleküle} 
	Moleküle besitzen oft kein permanentes magnetisches Moment. So summieren sich bei einer geradzahligen Menge an Elektronen die Drehimpulse zu null:
	$$
	...
	\end{align}
	wobei $\beta$ die magnetische Polarisierbarkeit ist. Mit Gleichung \eqref{eq:2.33} und \eqref{eq:2.34} ergibt sich
	$$m_\text{ind}=\beta B_0=\frac{\chi_\text m B_0}{N\mu_0}.
	$$
	Daraus folgt die
	\begin{important}
	...
	\paragraph{Beispiel 3: Benzol \ce{C6H6}} 
	Benzol ist stark anisotrop mit
	$$
	\mu_0 \beta_{\perp} = \SI{-152}{\cdot 10^{-36} \meter \cubed} \quad \text{und}\quad \mu_0 \beta_{ \|} = \SI{-62}{\cdot 10^{-36}\meter \cubed}.
	$$
	\autoref{fig:vl03_Benzol} erklärt die Bezeichnungen $\parallel$ und $\perp$ und zeigt die Orbitalstruktur von Benzol. $\pi$-Elektronen können leicht in der Ebene des Moleküls auf ein induziertes Magnetfeld reagieren, da sie entlang der konjugierten Doppelbindungen delokalisiert sind - es kommt zu sogenanntem \quickdef{molekularen Wirbelstrom}.\\
	...
	\begin{figure}[H]
		...
		\caption{links: Beschriftung der Richtungen $\parallel$ und $\perp$ im Benzol-Molekül.\\
		rechts: $\pi$-Orbitale des Benzol-Moleküls, in welchen sich die Elektronen frei um den Ring bewegen.}
		\label{fig:vl03_Benzol}
	\end{figure}
...
\subsection{Paramagnetische Moleküle}
	...
	\begin{equation}
		\label{eq:2.36}
		\vec{m'} = \frac{1}{3} \frac{\vec{m}_{p}\cdot\vec{B}_{0}}{\mathrm{k}_{\mathrm{B}} T} \vec{m}_{p}.
	\end{equation}
	\begin{verbal}
