...
Dabei ist $N$ die Zahl der Elementarzellen und $a$ die Gitterkonstante.
...
Die Lösung ist das Produkt aus einer ebenen Welle und einer gitterperiodischen Funktion.
...
Im Weiteren wird gezeigt, wie dies zu einer Energielücke im Energiespektrum der Elektronen führt.
...
Diese stehenden Wellen erhalten wir durch symmetrische und antisymmetrische Überlagerung:
$$
\begin{aligned}
...
\begin{figure}[H]
	...
	\caption{Die Abbildung zeigt die Lösungen für stehende Wellen $|\psi^{(+)}|^2$ und $|\psi^{(-)}|^2$. 
		Ihre physikalische Interpretation sind räumlich modulierte Ladungsdichten.
		Die Atome sind in der Skizze an 0, $a$, $2a$ lokalisiert.
		Daraus folgt, dass $|\psi^{(+)}|$ eine hohe Aufenthaltswahrscheinlichkeit am Kernort hat ($E_\mathrm{pot}$ klein) und $|\psi^{(-)}|$ eine geringe Aufenthaltswahrscheinlichkeit.
		Dies ist die Ursache der Energielücke.
	}
...
\begin{figure}[H]
	...
	\caption{Energie-Dispersionsrelation im Band. 
		In grün ist das erweiterte Zonenschema und in rot das reduzierte Zonenschema skizziert.
		Am Rand der Brillouinzone sind die Bänder flach (stehende Wellen) und es tritt eine Bandlücke auf.
		Die Energielücke ist so groß wie die zweifache Fourierkomponente des Kristallpotentials $E_\mathrm g = 2V_\mathrm g$.
	}
